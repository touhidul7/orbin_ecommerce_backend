    ...
    public function updateProduct(Request $request, $id)
{
    ...
    // Make sure you have column: recommended_product_id (nullable)
    $data->recommended_product_id = $request->filled('recommended_product_id')
        ? (int) $request->recommended_product_id
        : null;
    ...
    if ($request->hasFile('product_image')) {
        ...
        if (!empty($data->product_image)) {
            $oldPath = public_path('admin/product/' . $data->product_image);
        ...
        // keep old image if sent
        // (frontend sends existing_product_image)
        if ($request->filled('existing_product_image')) {
            $data->product_image = $request->existing_product_image;
        }
    }

    // ✅ Read existing gallery list from request (the ones user kept)
    $existingGalleryImages = [];
    if ($request->filled('existing_gallery_images')) {
        $existingGalleryImages = json_decode($request->existing_gallery_images, true);
        if (!is_array($existingGalleryImages)) $existingGalleryImages = [];
    } else {
        // fallback: use DB if request not present
        if (!empty($data->image_gallary)) {
            $tmp = json_decode($data->image_gallary, true);
            $existingGalleryImages = is_array($tmp) ? $tmp : [];
        }
    }

    // ✅ Delete removed gallery images from server
    if ($request->filled('removed_gallery_images')) {
        $removed = json_decode($request->removed_gallery_images, true);
        if (is_array($removed)) {
            foreach ($removed as $img) {
                $path = public_path('admin/product/gallery/' . $img);
                if (File::exists($path)) {
                    File::delete($path);
                }
            }
    ...
    // ✅ Final gallery = kept old + new uploads
    $finalGallery = array_values(array_merge($existingGalleryImages, $newFilenames));
    $data->image_gallary = json_encode($finalGallery);
